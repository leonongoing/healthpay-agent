...
def test_0g_storage_real():
    """Test real 0G Storage upload if credentials are available."""
    private_key = os.environ.get("ZG_PRIVATE_KEY", "")
    if not private_key or private_key.startswith("0x_your"):
        print("\n=== Test 3: 0G Storage Real Upload — SKIPPED (no ZG_PRIVATE_KEY) ===")
        return None

    from src.zero_g_storage import upload_to_0g, download_from_0g, _is_0g_available

    if not _is_0g_available():
        print("\n=== Test 3: 0G Storage Real Upload — SKIPPED (SDK not installed) ===")
        return None

    print("\n=== Test 3: 0G Storage Real Upload ===")
    test_data = {
        "test": True,
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "message": "HealthPay 0G integration test",
    }

    try:
        root_hash = upload_to_0g(data=test_data, filename="e2e_test.json")
        print(f"  Real upload OK — Root hash: {root_hash}")

        # Download and verify
        downloaded = download_from_0g(root_hash, "/tmp/test_0g_real_download.json")
        assert downloaded.get("test") is True
        print(f"  Real download + verify: ✓")
        return root_hash
...
=== Test 4: 0G Compute Client Init ===")
    ...
    print(f"  Has OpenAI fallback: {llm._openai_client is not None}")

    await llm.close()
    print(f"  Client init + close: ✓")


async def test_0g_compute_real():
    """Test real 0G Compute inference if credentials are available."""
    private_key = os.environ.get("ZG_COMPUTE_PRIVATE_KEY", "")
    if not private_key or private_key.startswith("0x_your"):
        print("\n=== Test 5: 0G Compute Real Inference — SKIPPED (no ZG_COMPUTE_PRIVATE_KEY) ===")
        return

    from src.zero_g_compute import ZeroGLLM

    print("\n=== Test 5: 0G Compute Real Inference ===")
    llm = ZeroGLLM(model="Qwen/Qwen2.5-7B-Instruct")

    try:
        if not llm.is_using_0g():
            print("  0G client not initialized, skipping real inference")
            return

        response = await llm.chat_completion(
            messages=[
                {"role": "system", "content": "You are a healthcare billing assistant."},
                {"role": "user", "content": "What is a CPT code 99213?"},
            ],
            temperature=0.3,
            max_tokens=200,
        )
        content = response.choices[0].message.content
        print(f"  Inference OK — Response: {content[:100]}...")
        print(f"  Model used: 0G Compute ✓")
    except Exception as e:
...
def test_mcp_integration():
    ...
    print("\n=== Test 6: MCP Server Import Check ===")
...
def main():
    ...
    # Test 5: Compute real
    try:
        asyncio.run(test_0g_compute_real())
        if not os.environ.get("ZG_COMPUTE_PRIVATE_KEY"):
            results["skipped"] += 1
        else:
            results["passed"] += 1
    except Exception as e:
